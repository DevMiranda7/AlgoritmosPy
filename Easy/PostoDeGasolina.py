...
print("ETANOL\nDIESEL\n")
TipoDeCombustivel = str(input("Qual combustível você deseja?"))
QuantidadeDeCombustivel = int(input("Quantos litros?"))

if TipoDeCombustivel == "ETANOL":
    if QuantidadeDeCombustivel > 0 and QuantidadeDeCombustivel <= 15:
        valorAPagar = QuantidadeDeCombustivel * 2
        desconto = (valorAPagar * 2)/100
        precoFinal = valorAPagar - desconto 
        print(f"R$: {precoFinal:.2f}")
    elif QuantidadeDeCombustivel > 15:
        valorAPagar = QuantidadeDeCombustivel * 2
        desconto = (valorAPagar * 4)/100
        precoFinal = valorAPagar - desconto 
        print(f"R$: {precoFinal:.2f}")
elif TipoDeCombustivel == "DIESEL":
    if QuantidadeDeCombustivel > 0 and QuantidadeDeCombustivel <= 15:
        valorAPagar = QuantidadeDeCombustivel * 1.70
        desconto = (valorAPagar * 3)/100
        precoFinal = valorAPagar - desconto 
        print(f"R$: {precoFinal:.2f}")
    elif QuantidadeDeCombustivel > 15:
        valorAPagar = QuantidadeDeCombustivel * 1.70
        desconto = (valorAPagar * 5)/100
        precoFinal = valorAPagar - desconto 
        print(f"R$: {precoFinal:.2f}")